...
def create_dnn_model(dimensions, numb_of_actions):
...
def supervised_learning(env, episodes, gamma, epsilon, alpha, epsilon_decay, min_epsilon, batch_size, update_target_network,
    ...
    dqn_model, optimizer, loss_function = create_dnn_model(env.dimensions, len(env.actions))
    ...
    for episode in range(episodes):
        ...
        while not env.done(state):
            ...
            reward = env.get_true_reward(state, action, next_state)
            return_ += reward
