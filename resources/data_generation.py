...
def create_correct_histories(env, epochs: int, result_as_unsorted_state_action_pairs: bool = False):
    ...
    for epoch in range(epochs):
        ...
        while not env.done(state):
            ...
            actions = env.get_correct_actions(state)
            if len(actions) == 0:
                break
            action = actions[randint(0, len(actions))]
            ...
            data_item.append({"state": state, "action": action_index,
                              "correct_actions": [env.action_to_int(a) for a in actions]})
